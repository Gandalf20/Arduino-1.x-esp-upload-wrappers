 ...
 */
public class ESP32LittleFS implements Tool {
  Editor editor;
  ...
  public String getMenuTitle() {
    return "ESP32 LittleFS Data Upload";
  }
  ...
  private void createAndUpload(){
    ...
    if (!partitionsFile.exists() || !partitionsFile.isFile()) {
      System.err.println();
      editor.statusError("LITTLEFS Error: partitions file " + partitions + ".csv not found!");
      return;
    }
    ...
    try {
      ...
      if(spiSize == 0){
        System.err.println();
        editor.statusError("LITTLEFS Error: partition size could not be found!");
        return;
      }
    } catch(Exception e){
      editor.statusError(e);
    ...
    if (!tool.exists() || !tool.isFile()) {
      ...
      if (!tool.exists()) {
        ...
        if (!tool.exists()) {
          tool = new File(PreferencesData.get("runtime.platform.path") + "/tools/mklittlefs", mkspiffsCmd);
          if (!tool.exists()) {
              System.err.println();
    ...
    String imagePath = getBuildFolderPath(editor.getSketch()) + "/" + sketchName + ".littlefs.bin";
    ...
    if(fileCount == 0 && JOptionPane.showOptionDialog(editor, message, title, JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[1]) != JOptionPane.YES_OPTION){
      System.err.println();
      editor.statusError("LITTLEFS Warning: mkspiffs canceled!");
      return;
    }

    editor.statusNotice("LITTLEFS Creating Image...");
    System.out.println("[LITTLEFS] data   : "+dataPath);
    System.out.println("[LITTLEFS] start  : "+spiStart);
    System.out.println("[LITTLEFS] size   : "+(spiSize/1024));
    ...
    if(isNetwork){
      ...
      System.out.println("[LITTLEFS] mode   : "+flashMode);
      System.out.println("[LITTLEFS] freq   : "+flashFreq);
      System.out.println();
